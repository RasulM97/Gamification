...
def snapshot(conn):
    ...
    return {name: sorted(repr(dict(row)) for row in conn.execute(sa.select(table)).mappings())
            for name,table in metadata.tables.items() if name not in ('alembic_version','approval_requests','approval_decisions','webhook_sources','rules','rule_candidates','policies','policy_decisions')}
