...
def test_n23_race_double_approve_single_outcome(client, auth):
    ...
    assert len(_notices(_state(client, auth['priya']), 'u-priya', 'REDEMPTION_APPROVED')) == 1  # no duplicate on retry
...
def test_n23_race_double_fulfill_single_record(client, auth):
    ...
    assert len(_notices(_state(client, auth['priya']), 'u-priya', 'REDEMPTION_FULFILLED')) == 1
...
def test_n23_notifications_fire_once_per_transition(client, auth):
    ...
    assert r.status_code == 200
    state = _state(client, auth['dana'])
    assert len(_notices(_state(client, auth['priya']), 'u-priya', 'REDEMPTION_APPROVED')) == 1
    assert len(_notices(_state(client, auth['jonas']), 'u-jonas', 'REDEMPTION_READY_FOR_FULFILLMENT')) == 1
    r = client.post('/api/redemptions/r2/approve', headers=auth['dana'])
