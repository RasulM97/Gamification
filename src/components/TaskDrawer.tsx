...
export function TaskDrawer({ taskId, onClose, onGo }: {
  ...
  if (!t || !canSeeTask(t, me)) return <Drawer open={!!taskId} onClose={onClose} title={tr('common.task')}>—</Drawer>
  ...
  const canHandoff = canReviewTask(state, t, me) && (t.status === 'IN_PROGRESS' || t.status === 'SUBMITTED') && t.ownerId && t.ownerId !== me.id
  const canReviewDecision = canReviewTask(state, t, me) && t.status === 'SUBMITTED' && !isOwner
  ...
  return (
    <Drawer open onClose={onClose} wide
      ...
      <TaskAccess key={t.id + JSON.stringify(t.viewerIds) + JSON.stringify(t.reviewerIds)} task={t} />
